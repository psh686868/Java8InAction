 ...
 * Date: 2017/12/20
 *
 * user : -server -Xmx200m -Xms20m -Xmn100m -Xss256k -XX:+UseParNewGC    -XX:+UseConcMarkSweepGC -XX:+UseCMSInitiatingOccupancyOnly  -XX:CMSInitiatingOccupancyFraction=75 -XX:+PrintGCDetails  -XX:+PrintGCDateStamps -XX:+PrintHeapAtGC
 *  -server 服务器模式
 *
...
public class CMSJVMDemo {
    ...
    public static void main(String[] args) {
        byte [] b1 = new byte[20 * _1MB];
        byte [] b2 = new byte[20 * _1MB];
        byte [] b3 = new byte[20 * _1MB];
        byte [] b4 = new byte[40 * _1MB];
        try {
            System.in.read();
        } catch (IOException e) {
            e.printStackTrace();
